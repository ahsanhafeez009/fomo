...
<div class="pcoded-content">
    ...
    <div class="card">
        <div class="card-block">
            <form action="{{url('/import-products')}}" method="post" enctype="multipart/form-data" id="import-products" class="dropzone">
                @csrf
                <div>
                </div>
            </form> 
...
<script type="text/javascript">
    ...
    let newarr = []
    var dropzone = new Dropzone('#import-products', {
        thumbnailWidth: 200,
        timeout: 18000000,
    ...
    function delete_product_file(id) {
        var result = confirm("Want to delete?");
        if (result) {
            $.ajax({
                type:"POST",
                dataType:"json",
                url: "{{url('/delete-product-file')}}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "data":  {id},
                },
    ...
    function accept_product_file(id) {
        var result = confirm("Want to Accept?");
        if (result) {
            $.ajax({
                type:"POST",
                dataType:"json",
                url: "{{url('/accept-product-file')}}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "data":  {id},
                },
