...
@section('content')
<style type="text/css">
    .matched_supplier_parent:has(.matched_supplier) {
      background: #92D050;
    }
</style>
                ...
                <table id="price_analysis_table" class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <? if (!empty(@$e['user_demand_qty'])) {
                                    echo @$e['user_demand_qty'];
                                }
                                ?>
                            </td>
                            <td>
                                <? if (!empty(@$e['lowest_price']['price_aed'])) {
                                    echo number_format((float)@$e['lowest_price']['price_aed'], 2, '.', '');
                                }
                                ?>
                            </td>
                            @foreach($suppliers as $supplier)
                                ...
                                @foreach ($e->get_supplier as $inSupplier)
                                    @if ($supplier->supplier_name === $inSupplier->supplier_name)
                                        <td class="matched_supplier_parent">
                                            <? if (!empty($inSupplier->price_aed)) {
                                            if (@$e['lowest_price']['price_aed'] == @$inSupplier->price_aed) { ?>
                                                <p class="matched_supplier"><?php echo number_format((float)$inSupplier->price_aed, 2, '.', ''); ?></p>
                                            <?php }else{ ?>
                                                <?= number_format((float)$inSupplier->price_aed, 2, '.', ''); ?>
                                            <?php }  
                                            }?> 
                                        </td>
