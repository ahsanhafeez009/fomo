...
class CompanyController extends Controller
{
    ...
    public function import_companies(Request $request)
    {
        $this->validate($request, [
            'raw_file' => 'required|file|mimes:xls,xlsx'
        ]);
        $the_file = $request->file('raw_file');
        try{
            ...
            $this->import_companies_to_db($data);
            return redirect()->route('create-company')->with('info-message','Companies are Imported Successfully');
        } catch (Exception $e) {
    ...
    function import_companies_to_db($resultant_array){
        ...
        if (is_array(@$company_names) || is_object(@$company_names)){
