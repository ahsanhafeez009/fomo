...
class ProductApiController extends Controller
{
    public function check_product_brand_names_cron_job()
    {
        $results = Product::select('brand_name')->groupBy('brand_name')->get()->toArray();
            ...
            foreach ($record_results as $key1 => $result_brand_name) {
                ...
                $B_name = Brands::select('brand_name')->where('brand_name', 'like', '%' . $new_result_brand_name . '%')->get();
    ...
    public function check_product_gender_cron_job()
    {
        $results = Product::select('product_gender')->groupBy('product_gender')->where('product_gender', '!=', Null)->get()->toArray();
            ...
            foreach ($record_results as $key1 => $result_gender) {
                $P_gender = ProductGender::select('gender')->where('gender', 'like', '%' . $result_gender . '%')->get();
    ...
    {
        $results = Product::select('*')->where('spell_check', '=', 0)->orderBy('id', 'ASC')->limit(1000)->get()->toArray();
    ...
    public function get_products_name_without_extra($resultant_products){
        ...
        foreach ($resultant_products as $key => $outerwrap) {
            if (str_contains($outerwrap['product_name'], "'") || str_contains($outerwrap['product_name'], "’")) { 
                $old_product_name = str_replace("'", ' ', $outerwrap['product_name']);
                $temp_product_name = str_replace("’", ' ', $old_product_name);
                $result = str_replace($remove_elements,"",strtolower($temp_product_name));
                $results[$key] = preg_replace('/^\s+|\s+$|\s+(?=\s)/', '', strtolower($result));
            }else{
                $result = str_replace($remove_elements,"",strtolower($outerwrap['product_name']));
                $results[$key] = preg_replace('/^\s+|\s+$|\s+(?=\s)/', '', strtolower($result));
            }
        }
        ...
        foreach ($resultant_products as $key5 => $outerwrap) {
            ...
            if (stripos(strtolower($outerwrap['product_name']), strtolower('edc')) !== false) {
                $resultant_product_names[$key5] = 'edc';
            }
        }

        foreach ($resultant_text as $key7 => $outer_wrap) {
            $product_record[$key7] = preg_replace('/^\s+|\s+$|\s+(?=\s)/', '', ucwords(@$outer_wrap) .' '. @$resultant_product_names[$key7]. ' '. @$bottle_sizes[$key7].'ml');
        }
